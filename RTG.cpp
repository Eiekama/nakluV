...
RTG::RTG(Configuration const &configuration_) : helpers(*this) {
	...
	if (configuration.headless) {
...

		{
			...
			std::cout << "Supported surface formats (" << count << "):\n";
			for (uint32_t i = 0; i < count; ++i) {
				auto const &format = formats[i];
				std::string index = " [" + std::to_string(i) + "] ";
				std::cout << std::setw(6) << std::right << index << std::setw(35) << std::left << string_VkFormat(format.format) << string_VkColorSpaceKHR(format.colorSpace) << "\n";
			}
			std::cout.flush();
		}
		{
			...
			std::cout << "Supported present modes (" << count << "):\n";
			for (uint32_t i = 0; i < count; ++i) {
	...
	{ //create the `device` (logical interface to the GPU) and the `queue`s to which we can submit commands:
		{ //look up queue indices:
			...
			for (uint32_t i = 0; i < count; ++i) {
				...
				std::cout << " [" << i << "] " << string_VkQueueFlags(queue_family.queueFlags) << "\n";
			}
...
static void scroll_callback(GLFWwindow *window, double xoffset, double yoffset) {
	...
	event.button.x = float(xoffset);
	event.button.y = float(yoffset);
...
void RTG::run(Application &application) {
	...
	while (configuration.headless || !glfwWindowShouldClose(window)) {
			...
			while (std::getline(std::cin, line)) {
				try {
					...
					if (type == "AVAILABLE") { //AVAILABLE dt [save.ppm]
						...
						if (headless_dt < 0)  throw std::runtime_error("dt less than zero");
		...
		if (configuration.headless) {
			...
			VK( vkWaitForFences(device, 1, &headless_swapchain[image_index].image_presented, VK_TRUE, UINT32_MAX) );
