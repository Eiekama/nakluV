...
Tutorial::Tutorial(RTG &rtg_) : rtg(rtg_) {
	...
	{ // create descriptor pool
		...
		VkDescriptorPoolCreateInfo create_info{
			...
			.maxSets = 3 * per_workspace, //two sets per workspace
	...
	{ // create object vertices
		...
		{ // klein bottle
			klein_bottle_vertices.first = uint32_t(vertices.size());

			constexpr float A = 2.5f;
			constexpr uint32_t U_STEPS = 100;
			constexpr uint32_t V_STEPS = 600;

			constexpr float U_REPEATS = 9.5f;
			constexpr float V_REPEATS = 5.0f;

			auto emplace_vertex = [&](uint32_t ui, uint32_t vi) {
				float ua = ui / float(U_STEPS) * 2.0f * float(M_PI);
				float va = (vi % V_STEPS) / float(V_STEPS) * 2.0f * float(M_PI);

				float k = A + std::cos(0.5f*ua)*std::sin(va) - std::sin(0.5f*ua)*std::sin(2.0f*va);
				float z = std::sin(0.5f*ua)*std::sin(va) + std::cos(0.5f*ua)*std::sin(2.0f*va);

				float dxdu = -A*std::sin(ua) - 0.5f*z*std::cos(ua);
				float dydu = A*std::cos(ua) - 0.5f*z*std::sin(ua);
				float dzdu = 0.5f*(std::cos(0.5f*ua)*std::sin(va) - std::sin(0.5f*ua)*std::sin(2.0f*va));
				float k1 = std::cos(0.5f*ua)*std::cos(va) - 2.0f*std::sin(0.5f*ua)*std::cos(2.0f*va);
				float dxdv = k1 * std::cos(ua);
				float dydv = k1 * std::sin(ua);
				float dzdv = std::sin(0.5f*ua)*std::cos(va) + 2.0f*std::cos(0.5f*ua)*std::cos(2.0f*va);
				...
				vertices.emplace_back( PosNorTexVertex{
					...
					.Normal{
						.x = vi >= V_STEPS/2 ? dydu*dzdv - dydv*dzdu : dydv*dzdu - dydu*dzdv,
						.y = vi >= V_STEPS/2 ? dzdu*dxdv - dzdv*dxdu : dzdv*dxdu - dzdu*dxdv,
						.z = vi >= V_STEPS/2 ? dxdu*dydv - dxdv*dydu : dxdv*dydu - dxdu*dydv,
					},
					.TexCoord{
						.s = vi >= V_STEPS/2 ? 1 - ui / float(U_STEPS) * U_REPEATS : ui / float(U_STEPS) * U_REPEATS,
						.t = vi >= V_STEPS/2 ? 1 - vi / float(V_STEPS) * V_REPEATS : vi / float(V_STEPS) * V_REPEATS,
					},
				});
			};

			for (uint32_t ui = 0; ui < U_STEPS; ++ui) {
				// flip backfacing to frontfacing
				for (uint32_t vi = 0; vi < V_STEPS/2; ++vi) {
					emplace_vertex(ui, vi);
					emplace_vertex(ui, vi+1);
					emplace_vertex(ui+1, vi);
				...
				// keep frontfacing
				for (uint32_t vi = V_STEPS/2; vi < V_STEPS; ++vi) {
					emplace_vertex(ui, vi);
					emplace_vertex(ui+1, vi);
					emplace_vertex(ui, vi+1);
			...
			klein_bottle_vertices.count = uint32_t(vertices.size()) - klein_bottle_vertices.first;
		}
	...
	{ // make some textures
		...
		{ // texture 2 will be black and white stripe pattern
			// actually make the texture:
			uint32_t size = 128;
			uint32_t scale = 8;
			std::vector< uint32_t > data;
			...
			for (uint32_t y = 0; y < size; ++y) {
				// float fy = (y + 0.5f) / float(size);
				uint32_t foo = uint32_t((float)y / size * scale);
				for (uint32_t x = 0; x < size; ++x) {
					// float fx = (x + 0.5f) / float(size);
					if (foo % 2 == 0) data.emplace_back(0x00000000);
					else data.emplace_back(0xffffffff);
				}
			}
	...
	{ // make a sampler for the texture
	...
	{ // create the texture descriptor pool
		...
		VK( vkCreateDescriptorPool(rtg_.device, &create_info, nullptr, &texture_descriptor_pool) );
	}
			...
			if (!object_instances.empty()) { // draw with the objects pipeline
				...
				for (ObjectInstance const &inst : object_instances) {
					...
					vkCmdBindDescriptorSets(
						...
						2,
						1,
						&texture_descriptors[inst.texture],
						0, nullptr
					);
...
void Tutorial::update(float dt) {
	...
	if (camera_mode == CameraMode::Scene) { // camera rotating around the origin: (technically i changed it to be stationary)
		float ang = 0.0f; //2.0f*float(M_PI) * 10.0f*(time/60.0f); // complete one revolution in 6 seconds
	...
	{ //static sun and sky:
		...
		world.SUN_ENERGY.r = r;//1.0f;
		world.SUN_ENERGY.g = g;//1.0f;
		world.SUN_ENERGY.b = b;//0.9f;
	}
	...
	{ // make some lines
		...
		// 	uint8_t mc = uint8_t(float(base) + mt2 * (255.0f - float(base)));
		// 	{
	...
	{ // make some objects
		...
		{
			{
				float ang = 2.0f*float(M_PI) * 10.0f*(time/60.0f);
				float ca = std::cos(ang);
				float sa = std::sin(ang);
				mat4 WORLD_FROM_LOCAL{
					ca,  -sa,   0.0f, 0.0f,
				...
				object_instances.emplace_back(ObjectInstance{
					.vertices = klein_bottle_vertices,
					.transform{
						.CLIP_FROM_LOCAL = CLIP_FROM_WORLD * WORLD_FROM_LOCAL,
						.WORLD_FROM_LOCAL = WORLD_FROM_LOCAL,
						.WORLD_FROM_LOCAL_NORMAL = WORLD_FROM_LOCAL,
